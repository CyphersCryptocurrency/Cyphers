...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // cyphers: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "cyphers Signed Message:\n";
...
{
    // cyphers: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // cyphers: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // cyphers: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // cyphers: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, cyphers - DarkGravity v2, written by Evan Duffield - evan@cyphers.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // cyphers: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // cyphers: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // cyphers: increase each by adding 2 to bitcoin's value.
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    // at a time so the setAdcyphnowns of the chosen nodes prevent repeats
    ...
    if (lockMain) {
        ...
        {
            {
                ...
                BOOST_FOREACH(CNode* pnode, vNodes)
                {
                    // Periodically clear setAdcyphnown to allow refresh broadcasts
                    if (nLastRebroadcast)
                        pnode->setAdcyphnown.clear();
        ...
        if (fSendTrickle)
        {
            ...
            BOOST_FOREACH(const CAddress& addr, pto->vAddrToSend)
            {
                ...
                if (pto->setAdcyphnown.insert(addr).second)
                {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("cyphersMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("cyphersMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static cyphersMiner(CWallet *pwallet)
{
    printf("cyphersMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cyphers-miner");
    ...
    try { loop {
        ...
        printf("Running cyphersMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("cyphersMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&cyphersMiner, pwallet));
}
